...
while not rospy.is_shutdown():
    if current_joint_positions is not None:
        ...
        position = np.zeros(6)
        position[:3] = np.array(T_numeric[:3, 3]).flatten()  # [x, y, z]
        R_numeric = np.array(T_numeric[:3, :3]).astype(np.float64)
        position[:3] = np.array(T_numeric[:3, 3]).astype(np.float64).flatten()  # [x, y, z]
        R_numeric = np.array(T_numeric[:3, :3]).astype(np.float64)
        # To avoid floating point errors, temporarily retain three decimal places
        R_numeric = np.round(R_numeric, decimals=3)
        # atan2 to calculate euler angle
        if abs(R_numeric[2, 0]) != 1:
            pitch = -math.asin(R_numeric[2, 0])
            cos_pitch = math.cos(pitch)
            roll = math.atan2(R_numeric[2, 1] / cos_pitch, R_numeric[2, 2] / cos_pitch)
            yaw = math.atan2(R_numeric[1, 0] / cos_pitch, R_numeric[0, 0] / cos_pitch)
        else:
            # Gimbal Lock condition
            yaw = 0
            if R_numeric[2, 0] == -1:
                pitch = math.pi / 2
                roll = yaw + math.atan2(R_numeric[0, 1], R_numeric[0, 2])
            else:
                pitch = -math.pi / 2
                roll = -yaw + math.atan2(-R_numeric[0, 1], -R_numeric[0, 2])

        position[3:] = [roll, pitch, yaw]
    ...
    rate.sleep()
        ...
        # Print logs to the terminal if necessary
